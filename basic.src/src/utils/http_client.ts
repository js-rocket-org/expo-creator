...
import { API_BASE_URL, API_KEY, StorageKeys } from '../constants';
import { APIRequest, APIResponse, HTTPClient, HttpCode, MapHeader, RequestType } from '../services/http_client';
...
const DEFAULT_HEADERS: MapHeader = {
...
// Returns a acces and refresh token pair or empty string if refresh failed
...
const jsonFetch = async (
  ...
  if (isAuthenticated && apiResponse.statusCode === HttpCode.UNAUTHORIZED) {
    const newTokenPair = await refreshTokens(authTokenPair);
    if (!newTokenPair) return RESPONSE_UNAUTHORIZED;
